...
## Get-UsersOfGroup
function Get_List_Of_User_Profiles ($strComputer){
    if (Ping-Server($strComputer))
        {
        write-host -foregroundcolor green "====== $strComputer User List ====="
        Get-WmiObject -ComputerName $strComputer win32_UserProfile | select LocalPath
    ...
    If($Item){
        Try{
            If (Test-Connection $Item -Count 1 -ErrorAction SilentlyContinue){
                Get_List_Of_User_Profiles -strComputer $Item
                }
            Else{
