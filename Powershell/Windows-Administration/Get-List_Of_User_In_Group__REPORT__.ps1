...
function Get_List_Of_Users ($strComputer){
    ...
    $membersofGroup = Listmembers $Group 
    
    $Results = @()
    #Build the Spreadsheet
    Foreach($User in $membersofGroup){
    $TempOBJ = New-Object PSObject
        $TempOBJ| Add-Member -MemberType NoteProperty -Name "Machine Name" -Value $strComputer
        $TempOBJ| Add-Member -MemberType NoteProperty -Name "Group" -Value $Groupname
        $TempOBJ| Add-Member -MemberType NoteProperty -Name "User or Security Group" -Value $User
    ...
    If($Item){
        Try{
            ...
            Else{
                ...
                $Error[0].ToString() | Out-Host
                Add-Content .\PingError.txt -Value $Item
                Write-Host -ForegroundColor Magenta "========^^^======="
                }
            }
        Catch [System.Management.Automation.PSArgumentException]{
            Write-Host -ForegroundColor Magenta "======Powershell Exception thrown on $Item ====="
            $_.PSArgumentException.Message | Out-Host 
             Add-Content .\PowershellError.txt $Item
            #$FailedItem1 = $_.PSArgumentException.ItemName | Out-Host | Out-File C:\Scripts\LocalAdmins\PowershellError.txt
            Write-Host -ForegroundColor Magenta "========^^^======="
            }
        Catch [System.Exception]{
            Write-Host -ForegroundColor Magenta "======System Exception thrown on $Item ====="
            $_.Exception.Message | Add-Content .\SystemError.txt | Out-Host
            Add-Content .\SystemError.txt $item
            #$FailedItem2 = $_.Exception.ItemName | Out-Host | Out-File C:\Scripts\LocalAdmins\SystemError.txt
            Write-Host -ForegroundColor Magenta "========^^^======="
            }
        }
    }
